...
void update_measurement(){
  if (sample_counter == n_samples){
      ...
      save_data_MEM(measurement_i,measurement_vel,measurement_temp, measurement_temp, duty);
      // Serial.print(measurement_vel);
...
void save_data_MEM(float corriente, float rpm, float empuje, float temperatura, float duty){
...
void extract_data_MEM(){
  for (int i = 0; i < EEPROM.length(); i+= 4){  //(apuntador)*4
    ...
    //Serial.print(i);
    Serial.print(EEPROM.get(i, resultado),3);
    Serial.print(", ");
    if (i % 20 == 0){
      Serial.println("\n");
    }
